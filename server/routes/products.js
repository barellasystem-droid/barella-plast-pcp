...
router.post('/', requireAuth, requireEdit(TAB), async (req, res) => {
  ...
  await db.query(
    `INSERT INTO products (code, name, molde, maquina, prod_dia, peso, cavidades, supplier_id)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
    [p.code, p.name, p.molde || '', p.maquina || '', Number(p.prodDia) || 0, Number(p.peso) || 0, Number(p.cavidades) || 0, p.supplierId || null]
  );
...
router.put('/:code', requireAuth, requireEdit(TAB), async (req, res) => {
  const p = req.body || {};
  await db.query(
    `UPDATE products SET name=$1, molde=$2, maquina=$3, prod_dia=$4, peso=$5, cavidades=$6, supplier_id=$7 WHERE code=$8`,
    [p.name, p.molde || '', p.maquina || '', Number(p.prodDia) || 0, Number(p.peso) || 0, Number(p.cavidades) || 0, p.supplierId || null, req.params.code]
  );
